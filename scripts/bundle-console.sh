...
copy_if_exists "console-next/.next/standalone/." "$STAGE_DIR"
copy_if_exists "console-next/.next/static" "$STAGE_DIR/.next/static"
...
if [ -x "$ROOT_DIR/node-runtime/bin/node" ]; then
  exec "$ROOT_DIR/node-runtime/bin/node" "$ROOT_DIR/server.js"
fi
